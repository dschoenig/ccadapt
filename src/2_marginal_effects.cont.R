...
for(i in seq_along(vars.irt)) {
  ...
  if(var.type == "continuous") {
    ...
    par.name <- paste0("eta_", var.foc)

    irt.p <- fixef(mod.irt, summary = FALSE)
    irt.g <- ranef(mod.irt, groups = "item", summary = FALSE)

    var.eff.p <- irt.p[,par.name]
    var.eff.g <- irt.g[[1]][,,par.name]

    var.eff.comb <-
      apply(var.eff.g, 2, \(x) x+var.eff.p) |>
      as.data.table() |>
      _[, .draw := 1:.N] |>
      melt(id.vars = ".draw",
           variable.name = "resp",
