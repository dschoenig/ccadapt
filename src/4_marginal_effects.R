...
#     variables. Should be set to same value as in `1_models_varsel.R`, and
#     `3_irt_2pl.R`.
# `pred.scales`:
...
# `mar.type`:
#     Type of marginal effect. Either `cf` for full counterfactuals
#     (computationally expensive), or marginal effects at the mean (for which
#     the interpretation does not correspond to the surveyed propulation).
# `cont.pred.n`:
#     Number of evenly spaced values at which to evaluate continuous covariates.
# `slope.res`:
#     How to numerically differentiate continuous variables to obtain slopes.
#     Either `coarse` to simply differentiate between neighbouring values at
#     which the covariate was evaluated, or `fine` to use smaller increments
#     between these values.
# `cont.diff.frac`:
#     For `slope.res = "fine"`, chooses the resolution of the differentiation
#     grid. Distance to neighbouring values are divided by `cont.diff.frac` to
#     obtain the increment for differentiation, i.e. larger values for
#     `cont.diff.frac` correspond to a finer grid.
# `var.log`:
#     Character vector indicating the continuous variables that are to be
#     long-transformed before evaluation.
# `draw.ids`:
...
file.irt.pred.all <- paste0(path.results.irt, "predictions.all.", mar.type, ".rds")
...
# Avoid recalculating predictions if possible
...
# Handle factor levels and reference categories
